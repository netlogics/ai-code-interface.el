...
** Main branch change
- Stabilize Ghostel image previews during scrolling and redraws, by fxa
- Add lightweight grill-me prompt flow
- Fix Ghostel input jitter caused by inline image anchoring, by fxa
- Support selected region as context in TDD write-test stage

** 1.89
- Fix: Avoid autoload-time evaluation of harness test suffix, reported by fxa
- Fix Ghostel startup blocked by linum/margin overlay false positive
- Preserve Ghostel links during animated redraws
- Fix Ghostel IME preedit flicker, by fxa
- Fix wrapped session link hover coverage in Ghostel, by fxa
- Chore: Require deterministic randomness in AI-generated test prompts

** 1.88
- Fix: Fix wrapped session link underlines, by fxa
- Feat: Add Ghostel image previews, by fxa
...
- Chore: Prompt for working directory on prefixed AI CLI start/resume, reported by isoboroff
- Fix: [codex] Normalize vterm dim SGR foreground, by cxa
...
** 1.86
- Fix C-c C-c override in org task files
- Feat: add task file agent handoff across AI backends
...
- Fix: improve antigravity cli backend prompt sending speed.
...
- Fix: Address feedbacks in https://github.com/melpa/melpa/pull/10028
...
** 1.82
- Feat: Quick prompts 
- Feat: Add Antigravity CLI backend support
- Fix: Simplify ai-code-plain-read-string to use read-string
...
- Fix: Rebind file buffers when starting a new AI session
- Feat: Add dedicated AI note search with a separate menu key
- Feat: Redesign ai-code-take-notes for Org and Org-roam integration
...
- Chore: Add AI session side-panel resize shortcuts, inside AI coding session buffer
  - C-. to increase AI coding session panel size, C-, to decrease
- Refactor: Move more code from ai-code.el to ai-code-harness.el
- Feat: Add difftastic integration for AI-generated code review
- Chore: Support resuming AI sessions from a selected UUID
...
- Refactor: Extract terminal specific variable / logic to their corresponding .el file
- Cleanup: Remove pytest / jest / ert based test runner
- Feat: Add resolve merge conflict AI tool for pull requests
- Fix: Subtle issues in claude code backend infra
- Feat: Add emacs debugging and inspection MCP tools
- Chore: Use ghostel-exec for Ghostel backend startup

** 1.71
- Feat: Add get_variable_value MCP tool
- Fix: Resolve github-copilot-cli scroll back issue
- Chore: bypass gptel prompt classification for existing menu items (code change, ask question, explain)
- Fix: Scroll back issue for vterm & eat
- Feat: Add optional editor-session MCP tools
- Feat: Add terminal backend selection menu item to switch between vterm, eat, and ghostel

** 1.70
- Feat: Add experimental Ghostel backend infra support
- Feat: Supporting search content inside task org files
